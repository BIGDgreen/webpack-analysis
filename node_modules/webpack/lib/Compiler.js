...
class Compiler {
	 ...
	 */
	run(callback) {
		console.log('🌺 25. 执行compiler.run，得到回调函数 callback 作为参数: ', callback);
		if (this.running) {
			// 已经在运行，抛出并发异常
		...
		const finalCallback = (err, stats) => {
			...
			this.running = false;
			if (err) {
				console.log(magenta('    🐶 hooks: failed call'));
				this.hooks.failed.call(err);
			}
			if (callback !== undefined) callback(err, stats);
			console.log(magenta('    🐶 16. hooks: afterDone call'));
		...
		const onCompiled = (err, compilation) => {
			...
			console.log(' ---------------------- onCompiled start ---------------------- ');
			console.log(magenta('    🐶 13. hooks: shouldEmit call, 为 false 时继续'));
			if (this.hooks.shouldEmit.call(compilation) === false) {
				console.log('继续');
				compilation.startTime = startTime;
				compilation.endTime = Date.now();
				const stats = new Stats(compilation); // 记录统计的 compilation 信息
				console.log(magenta('    🐶 12. hooks: done callAsync'));
			...
			process.nextTick(() => {
				console.log(' ---------------------- onCompiled nextTick start ---------------------- ');
				logger = compilation.getLogger("webpack.Compiler");
				logger.time("emitAssets");
				this.emitAssets(compilation, err => {
					logger.timeEnd("emitAssets");
					if (err) return finalCallback(err);
					console.log(magenta('    🐶 14. hooks: needAdditionalPass call, 判断为 true 时继续'));
					if (compilation.hooks.needAdditionalPass.call()) {
						...
						// 调用done生命周期
						console.log(magenta('    🐶 13. hooks: done callAsync'));
						this.hooks.done.callAsync(stats, err => {
							logger.timeEnd("done hook");
							if (err) return finalCallback(err);

							console.log(magenta('    🐶 14. hooks: additionalPass callAsync'));
					...
					this.emitRecords(err => {
						...
						logger.time("done hook");
						const stats = new Stats(compilation);
						console.log(magenta('    🐶 15. hooks: done callAsync'));
						this.hooks.done.callAsync(stats, err => {
							logger.timeEnd("done hook");
							if (err) return finalCallback(err);
					...
					console.log(' ---------------------- onCompiled nextTick end ---------------------- ');
				});
			});
			console.log(' ---------------------- onCompiled end ---------------------- ');
		};

		const run = () => {
			// 调用 beforeRun 钩子的回调，会清空前文的 inputFile 文件系统的缓存和内容
			console.log(magenta('    🐶 4. hooks: beforeRun callAsync'));
			this.hooks.beforeRun.callAsync(this, err => {
				if (err) return finalCallback(err);

				// 开始执行 run 这个钩子的逻辑，判断是否有对应的缓存记录
				console.log(magenta('    🐶 5. hooks: run callAsync'));
				this.hooks.run.callAsync(this, err => {
					...
					this.readRecords(err => {
						// 读缓存
						if (err) return finalCallback(err);
						console.log('🌺 26. 直接执行整个构建流程，传入会回调的逻辑，为 compile 传入参数, onCompiled', onCompiled);
						this.compile(onCompiled);
					});
				});
			});
...
${other}`);
	...
	newCompilation(params) {
		...
		console.log(magenta('    🐶 8. hooks: thisCompilation call'));
		this.hooks.thisCompilation.call(compilation, params);
		console.log(magenta('    🐶 9. hooks: compilation call'));
	...
	// 真正编译的过程
	compile(callback) {
		console.log("🌺 26. 实例化核心的工厂对象, 传入 callback: ", callback);
		// 实例化核心的工厂对象
		const params = this.newCompilationParams();
		console.log(magenta('    🐶 6. hooks: beforeCompile callAsync'));
		this.hooks.beforeCompile.callAsync(params, err => {
			if (err) return callback(err);

			console.log(magenta('    🐶 7. hooks: compile call'));
			this.hooks.compile.call(params);

			console.log("🌺 27. 创建 compilation 对象");
			const compilation = this.newCompilation(params);

			const logger = compilation.getLogger("webpack.Compiler");

			logger.time("make hook");
			console.log(magenta('    🐶 10. hooks: make call'));
			this.hooks.make.callAsync(compilation, err => {
				logger.timeEnd("make hook");
				if (err) return callback(err);

				logger.time("finish make hook");
				console.log(magenta('    🐶 11. hooks: finishMake call'));
					...
					process.nextTick(() => {
						...
						compilation.finish(err => {
							...
							compilation.seal(err => {
								...
								console.log(magenta('    🐶 12. hooks: afterCompile call'));
