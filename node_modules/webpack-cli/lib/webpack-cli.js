...
class WebpackCLI {
    ...
    async _baseResolver(cb, parsedArgs, strategy) {
        // console.log("进入baseResolver", cb, this.compilerConfiguration);
    ...
    async runOptionGroups(parsedArgs) {
        await Promise.resolve()
            ...
            .then(() => {
                this._baseResolver(resolveMode, parsedArgs);
            })
    ...
    async run(args) {
        console.log("🌺 进入run，接收参数: ", args);
        console.log("--------------------------- 读取和合并配置文件start ---------------------------------");
        await this.runOptionGroups(args);
        console.log("🌺 执行结束runOptionGroups, 得到 compilerConfiguration(options): ", this.compilerConfiguration, ', outputConfiguration(outputOptions): ', this.outputConfiguration);
        console.log("--------------------------- 读取和合并配置文件end ---------------------------------");
        let compiler;
        ...
        if (isRawOutput) {
            ...
            console.log("🌺 options.json 不存在，创建cli插件，并收集到options.plugins数组，options.plugins: ", options.plugins);
        }
        ...
        console.log("🌺 创建compiler，传入options: ", options, ' callback: ', callback);
        console.log(bgBlue('---------------------------------------创建compiler开始--------------------------------------'));
        compiler = this.createCompiler(options, callback);
        console.log(bgBlue('---------------------------------------创建compiler结束--------------------------------------'));
        console.log("🌺 得到compiler ");
