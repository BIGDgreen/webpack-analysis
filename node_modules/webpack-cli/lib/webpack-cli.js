...
class WebpackCLI {
    ...
    async run(args) {
        console.log("🌺 8. 进入run，接收参数: ", args);
        console.log("--------------------------- 读取和合并配置文件start ---------------------------------");
        await this.runOptionGroups(args);
        console.log("--------------------------- 读取和合并配置文件end ---------------------------------");
        console.log("🌺 16. 执行结束runOptionGroups, 得到 compilerConfiguration（options）: ", this.compilerConfiguration, 'outputConfiguration（outputOptions）: ', this.outputConfiguration);
        let compiler;
        ...
        if (isRawOutput) {
            ...
            console.log("🌺 17. options.json 不存在，创建cli插件，并收集到options.plugins数组，options.plugins: ", options.plugins);
        }
        ...
        console.log("🌺 18.  创建compiler，传入options: ", options, ' callback: ', callback);
        compiler = this.createCompiler(options, callback);
        console.log(bgBlue('---------------------------------------创建compiler结束--------------------------------------'));
        console.log("🌺 29.  得到compiler ");
