...
const runCLI = async (cliArgs) => {
    console.log("🌺 进入 runCLI 函数，参数: ", cliArgs);
    const parsedArgs = argParser(core, cliArgs, true, process.title);
    console.log("🌺 参数解析，解析后的参数: ", parsedArgs);
    const commandIsUsed = isCommandUsed(cliArgs);
    if (commandIsUsed) {
        console.log("🌺 使用命令行保留字命令，停止构建 cli 实例");
        return;
    }

    try {
        // Create a new instance of the CLI object
        console.log("🌺 创建 WebpackCLI 实例");
        ...
        if (parsedArgs.unknownArgs.length > 0) {
            ...
            console.log("🌺 将不是以 - 开头的命令 push 进 entry 数组");
        ...
        if (parsedArgs.unknownArgs.length > 0) {
            console.log("🌺 出现未知错误命令，为错误命令提供修改建议");
        ...
        console.log("🌺 异步执行cli，传入剔除unknownArgs和加入entry后的参数: " , parsedArgsOpts, 'core');
        console.log(bgBlue('---------------------------------------准备执行cli.run--------------------------------------'));
        await cli.run(parsedArgsOpts, core);
        console.log('🌺 cli.run 执行结束, webpack构建流程结束 🎉');
