...
const cliFlags = require('webpack-cli/lib/utils/cli-flags');
...
const resolveConfigFiles = async (args) => {
    console.log("🌺 9. 开始读取用户配置，得到参数: ", args);
    const { config, mode } = args;

    if (config && config.length > 0) {
        console.log("🌺 10. 读取并解析用户自定义配置文件", config);
        ...
        console.log("🌺 11.  解析完成用户自定义配置文件，得到", opts);
        return;
    }
    ...
    console.log("🌺 10. 用户未指定配置文件，采用默认配置文件", tmpConfigFiles);

    const configFiles = tmpConfigFiles.map(requireConfig);
    console.log("🌺 11. 读取默认配置文件 configFiles :", configFiles);

    if (configFiles.length) {
        const defaultConfig = configFiles.find((p) => p.path.includes(mode) || p.path.includes(modeAlias[mode]));
        console.log("🌺 12. 读取配置文件下特定环境的配置，如 webpack.config.prod.js/webpack.config.dev.js :", defaultConfig);

        if (defaultConfig) {
            opts = await finalize(defaultConfig, args, true);
            console.log("🌺 13. 有特定环境下的配置，得到最终opts: ", opts);
            return;
        }
        ...
        console.log("🌺 13. 没有特定环境下的配置，取 configFiles 中的第一项，得到最终opts: ", opts);
...
module.exports = async (args) => {
    ...
    console.log("🌺 14. resolveConfigFiles 解析配置执行结束，得到opts:", opts);
    await resolveConfigMerging(args);
    console.log("🌺 15. resolveConfigMerging 合并配置执行结束，得到opts:", opts);
    return opts;
};
